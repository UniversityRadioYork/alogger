...
class Logger:
  ...
  def __init__(self, 
    ...
    # check if root
    if os.getuid() != 0:
      stderr.write("Error: need to initialise as superuser\n")
      exit(0)

    # make location if needed
    if not os.path.isdir(self.__location):
      os.mkdirs(self.__location, 0640)
  
    # get owner and group info
    self.__owner = pwd.getpwnam(owner)
    self.__group = grp.getgrnam(group)
    
    # change folder owner
    os.chown(self.__location, self.__owner.pw_uid, self.__group.gr_gid)

    # change running permissions
    os.setgid(self.__group.gr_gid)
    os.setuid(self.__owner.pw_uid)
